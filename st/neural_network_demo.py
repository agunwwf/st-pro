...
def quiz_section():
    # 1. 结构化的高质量题库（带有 A/B/C/D 前缀，并丰富了干扰选项与专业解析）
    NEURAL_QUIZ_DATA = [
        {
            "question": "1. 传统人工神经网络 (ANN) 的最基本组成部分包含哪些？",
            "options": [
                "A. 卷积层、池化层、全连接层",
                "B. 输入层、隐藏层、输出层，以及连接权重 (Weights) 和激活函数",
                "C. 特征提取器、分类器、优化器",
                "D. 编码器 (Encoder)、解码器 (Decoder) 和注意力机制"
            ],
            "answer": "B. 输入层、隐藏层、输出层，以及连接权重 (Weights) 和激活函数",
            "explanation": "选项 A 和 D 分别是 CNN (卷积神经网络) 和 Transformer 的核心组件。最基础的神经网络（多层感知机 MLP）是由输入层、若干隐藏层、输出层组成的，神经元之间通过权重连接，并由激活函数输出。"
        },
        {
            "question": "2. 在神经网络中，激活函数 (Activation Function) 的核心作用是什么？",
            "options": [
                "A. 显著加速网络的训练和计算速度",
                "B. 自动对输入数据进行标准化降维处理",
                "C. 为网络引入非线性，使其能够学习和拟合复杂的非线性模式",
                "D. 防止模型在训练数据上发生过拟合现象"
            ],
            "answer": "C. 为网络引入非线性，使其能够学习和拟合复杂的非线性模式",
            "explanation": "如果没有激活函数（或者仅使用线性激活函数），无论神经网络有多少层，其本质上仍然只是一个线性回归模型，只能解决线性可分问题。激活函数（如 ReLU、Sigmoid）打破了这种线性叠加，赋予了网络强大的拟合能力。"
        },
        {
            "question": "3. 反向传播算法 (Backpropagation) 在模型训练中扮演什么角色？",
            "options": [
                "A. 利用链式法则，计算损失函数对每个参数的梯度，指导优化器更新权重",
                "B. 将输入数据从前向后依次传递，最终计算出网络的预测输出",
                "C. 自动为隐藏层选择最佳的激活函数和神经元数量",
                "D. 在前向传播受阻时，提供一条备用的数据流动路径"
            ],
            "answer": "A. 利用链式法则，计算损失函数对每个参数的梯度，指导优化器更新权重",
            "explanation": "前向传播用于计算预测值和损失；反向传播则是利用微积分中的链式法则，从输出层反向逐层计算梯度。拿到梯度后，再交由梯度下降等优化算法去更新网络权重。"
        },
        {
            "question": "4. 当神经网络发生“过拟合” (Overfitting) 时，通常会有怎样的表现？",
            "options": [
                "A. 训练集准确率低，测试集准确率也低",
                "B. 训练集准确率高，测试集准确率极低",
                "C. 训练集准确率低，测试集准确率异常高",
            ...
            "answer": "B. 训练集准确率高，测试集准确率极低",
            "explanation": "过拟合意味着神经网络“死记硬背”了训练数据中的细节和噪声，导致它在训练集上表现得像个学霸，但缺乏泛化能力 (Generalization)，一遇到没见过的测试数据就彻底考砸。"
        },
        {
            "question": "5. “深度学习” (Deep Learning) 与传统较浅的神经网络的主要区别是什么？",
            "options": [
                "A. 深度学习不需要使用激活函数",
                "B. 深度学习完全抛弃了反向传播算法",
                "C. 深度学习只能处理图像数据，不能处理文本",
                "D. 深度学习包含更多的隐藏层，能够学习更加抽象的层次化特征"
            ],
            "answer": "D. 深度学习包含更多的隐藏层，能够学习更加抽象的层次化特征",
            "explanation": "“深”指的是网络的层数多。传统网络通常只有1-2个隐藏层，而深度学习网络（如 ResNet）可以达到成百上千层。这使得网络能够进行“层次化特征提取”——底层提取边缘，中层提取纹理，高层提取语义概念。"
        }
    ]
    ...
    render_quiz_component(
        module_key="neural",
        title="🎯 神经网络概念与原理测验",
        description="本测验将检验你对神经网络的拓扑结构、激活函数的非线性作用以及反向传播机制的理解。完成后点击提交，测试成绩及错题解析将同步保存。",
        quiz_data=NEURAL_QUIZ_DATA
    )

    return "概念测验模块: 用户正在进行 神经网络 综合选择题测验"
