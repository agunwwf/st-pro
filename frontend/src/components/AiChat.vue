...
const API_URL = `${(import.meta.env.VITE_BASE_URL || '').replace(/\/$/, '')}/api/chat/stream`
const GREETING = '你好！我是 学习平台 的智能客服凡可 😊，有什么可以帮你的吗？'
const SUGGESTIONS = ['逻辑回归', '神经网络', '线性回归', '文本分析']
  ...
  es.onerror = () => {
    ...
    if (!messages.value[lastIdx].content) {
      messages.value[lastIdx].content =
          '暂时无法获取回复，请确认后端已启动且网络正常；若仍失败，请刷新页面后重试。'
    }
